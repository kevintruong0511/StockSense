...
export function SourceChips({ items }) {
  ...
  return (
    <div className="mt-2.5 flex flex-wrap items-center gap-1.5 border-t border-slate-100 pt-2 dark:border-slate-800">
      <span className="mr-0.5 text-[11px] font-semibold text-slate-400 dark:text-slate-500">Nguồn</span>
      {items.map((s, i) => (
        <a
          key={i}
          href={s.url}
          ...
          rel="noreferrer"
          title={s.title || s.url}
          className="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-50 px-2 py-0.5 text-[11px] text-slate-600 no-underline transition-colors hover:bg-slate-100 hover:text-slate-900 dark:border-slate-700 dark:bg-slate-800 dark:text-slate-400 dark:hover:bg-slate-700 dark:hover:text-white"
        >
          <img
...
export function StatusIndicator({ status }) {
  ...
  return (
    <div className="flex items-center gap-2 text-slate-500 dark:text-slate-400">
      <Icon size={15} className="shrink-0 animate-pulse text-blue-500 dark:text-blue-400" />
      <span className="text-sm">
        {label}
        {isSearch && status?.query && (
          <span className="font-medium text-slate-700 dark:text-slate-300"> “{status.query}”</span>
        )}
      </span>
