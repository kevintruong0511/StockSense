...
export default function Landing({ onLogin, onRegister, onPricing, onStart, onSelectTicker }) {
  return (
    <div className="flex min-h-screen flex-col bg-gradient-to-b from-slate-900 to-slate-950">
      {/* top bar */}
      <header className="mx-auto flex w-full max-w-[1200px] items-center justify-between px-10 py-5 max-sm:px-5">
        ...
        <nav className="flex items-center gap-2.5 sm:gap-4">
          ...
          <button
            onClick={onLogin}
            className="rounded-[10px] border-[1.5px] border-slate-700 bg-slate-900 px-[18px] py-2.5 text-sm font-semibold text-slate-200 transition-colors hover:border-slate-600 hover:bg-slate-800 max-sm:px-3.5"
          >
            Đăng nhập
          </button>
          <button
      ...
      <section className="mx-auto w-full max-w-[1000px] px-10 pb-10 pt-14 text-center max-sm:px-5">
        <div className="mb-[26px] inline-flex items-center gap-2 rounded-full border border-blue-500/20 bg-blue-500/10 px-3.5 py-1.5 text-[13px] font-semibold text-blue-400">
          <Sparkle size={15} />
          Phân tích cổ phiếu Việt bằng AI phân tích chuyên nghiệp
        </div>
        <h1 className="m-0 mb-5 text-[56px] font-extrabold leading-[1.05] tracking-[-0.03em] text-white [text-wrap:balance] max-md:text-[40px]">
          Hiểu sâu cổ phiếu Việt
          <br />
          trước khi xuống tiền
        </h1>
        <p className="mx-auto mb-9 max-w-[640px] text-[19px] leading-[1.55] text-slate-400 [text-wrap:pretty]">
          Tích hợp <strong className="font-bold text-white">AI chuyên sâu</strong> phân tích thị
          trường, phân tích từng mã cổ phiếu và đưa ra dự đoán với{' '}
          <strong className="font-bold text-white">độ chính xác cao</strong>. Đăng ký để trải
        ...
        <div className="mb-9 flex flex-wrap items-center justify-center gap-3">
          ...
          <button
            onClick={onLogin}
            className="rounded-[12px] border-[1.5px] border-slate-700 bg-slate-900 px-7 py-[15px] text-[16px] font-semibold text-slate-200 transition-colors hover:border-slate-600 hover:bg-slate-800"
          >
            Đăng nhập
          </button>
        </div>
        ...
        <div className="relative mx-auto mb-[18px] max-w-[560px]">
          <div className="flex items-center gap-2.5 rounded-[14px] border-[1.5px] border-slate-700 bg-slate-900 p-1.5 pl-4 shadow-[0_8px_24px_rgba(0,0,0,.25)]">
            <Search size={20} className="text-slate-500" />
            <input
              placeholder="Nhập mã cổ phiếu: FPT, HPG, VNM…"
              className="min-w-0 flex-1 border-0 bg-transparent text-base text-slate-100 outline-none placeholder:text-slate-500"
            />
            <button
              onClick={onStart}
        ...
        <div className="flex flex-wrap justify-center gap-2">
          <span className="mr-0.5 text-[13px] text-slate-500">Phổ biến:</span>
          {CHIPS.map((code) => (
            <button
              key={code}
              onClick={() => onSelectTicker(code)}
              className="tnum rounded-lg border border-slate-700 bg-slate-900 px-3 py-[5px] text-[13px] font-semibold text-slate-300 transition-colors hover:border-slate-600"
            >
              {code}
            </button>
          ))}
      ...
      <section className="mx-auto w-full max-w-[1120px] px-10 pb-5 pt-6 max-sm:px-5">
        <div className="mb-6 text-center">
          <h2 className="m-0 text-[30px] font-extrabold tracking-[-0.02em] text-white max-md:text-[24px]">
            Mọi thứ bạn cần để nghiên cứu cổ phiếu
          </h2>
          <p className="mx-auto mt-2 max-w-[560px] text-[15px] text-slate-400">
        ...
        <div className="grid grid-cols-[repeat(auto-fit,minmax(280px,1fr))] gap-5">
          {FEATURES.map((f) => {
            ...
            return (
              <div
                key={f.title}
                className="rounded-2xl border border-slate-800 bg-slate-900 p-[26px] transition-colors hover:border-slate-700"
              >
                <div
                ...
                <h3 className="m-0 mb-2 text-[17px] font-bold tracking-[-0.01em] text-white">{f.title}</h3>
                <p className="m-0 text-sm leading-[1.55] text-slate-400">{f.desc}</p>
              </div>
            )
          })}
        </div>
      ...
      <section className="mx-auto mt-8 w-full max-w-[1120px] px-10 max-sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-8 rounded-[18px] border border-slate-800 bg-slate-900/60 px-10 py-[34px] max-sm:px-6">
          <div>
      ...
      <section className="mx-auto mt-6 w-full max-w-[1120px] px-10 max-sm:px-5">
        <div className="flex flex-col items-center gap-5 rounded-[18px] border border-blue-500/20 bg-blue-500/10 px-10 py-10 text-center max-sm:px-6">
          <div className="flex flex-wrap items-center justify-center gap-x-5 gap-y-1.5 text-[13.5px] font-semibold text-blue-400">
          ...
          <h2 className="m-0 text-[28px] font-extrabold tracking-[-0.02em] text-white max-md:text-[22px]">
          ...
          <div className="flex flex-wrap items-center justify-center gap-3">
            ...
            <button
              onClick={onLogin}
              className="rounded-[12px] border-[1.5px] border-blue-500/30 bg-slate-900 px-8 py-[15px] text-[16px] font-semibold text-blue-400 transition-colors hover:border-blue-400/50"
            >
              Tôi đã có tài khoản
            </button>
          </div>
      ...
      <footer className="mx-auto mb-0 mt-auto w-full max-w-[1120px] p-10 max-sm:px-5">
        <div className="flex flex-wrap items-center justify-between gap-4 border-t border-slate-800 pt-6">
          <div className="flex items-center gap-[9px]">
            ...
            <span className="text-[15px] font-bold text-white">StockSense VN</span>
          </div>
