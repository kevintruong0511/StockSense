...
function Field({ label, icon: Icon, children }) {
  return (
    <label className="block">
      ...
      <div className="relative rounded-[11px] border-[1.5px] border-slate-200 bg-white transition-colors focus-within:border-blue-500 dark:border-slate-700 dark:bg-slate-900 dark:focus-within:border-blue-500">
        {Icon && (
          <Icon
            size={18}
...
const inputCls = (hasIcon) =>
  `w-full border-0 bg-transparent py-3 ${hasIcon ? 'pl-11' : 'pl-4'} pr-4 text-[15px] text-slate-900 outline-none placeholder:text-slate-400 dark:text-slate-100 dark:placeholder:text-slate-500`
...
function PanelHeader({ title, desc }) {
  return (
    <div className="mb-5">
      <h2 className="m-0 text-[19px] font-bold tracking-[-0.01em] text-slate-900 dark:text-white">{title}</h2>
      {desc && <p className="m-0 mt-1 text-[13px] text-slate-500 dark:text-slate-400">{desc}</p>}
    </div>
  )
}
  ...
  return (
    <div className="grid gap-5">
      <section className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
        ...
        <div className="mb-5 flex items-center gap-3 rounded-xl bg-slate-50 px-4 py-3 dark:bg-slate-800/60">
          ...
          <div className="min-w-0">
            <div className="truncate text-sm font-bold text-slate-900 dark:text-white">{user?.name || 'Nhà đầu tư'}</div>
            <div className="truncate text-[12.5px] text-slate-500 dark:text-slate-400">{user?.email}</div>
          </div>
        </div>
        ...
        <form onSubmit={saveName} className="flex flex-col gap-3">
          ...
          <Field label="Email (không thể đổi)" icon={Mail}>
            <input
              ...
              value={user?.email || ''}
              disabled
              className={inputCls(true) + ' text-slate-400 dark:text-slate-500'}
            />
          </Field>
        ...
        <form onSubmit={savePw} className="flex flex-col gap-3">
          ...
          <div>
            <button
              type="submit"
              disabled={savingPw}
              className="rounded-[11px] bg-slate-900 px-5 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-60 dark:bg-slate-700 dark:hover:bg-slate-600"
            >
    ...
    <section className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="flex flex-wrap gap-3">
        <a
          ...
          className="inline-flex items-center gap-2 rounded-[11px] border-[1.5px] border-slate-200 bg-white px-4 py-2.5 text-sm font-semibold text-slate-800 transition-colors hover:border-slate-300 hover:bg-slate-50 dark:border-slate-700 dark:bg-slate-900 dark:text-slate-200 dark:hover:border-slate-600 dark:hover:bg-slate-800"
        >
...
function AppearancePanel({ theme = 'dark', onThemeChange }) {
  return (
    <section className="rounded-2xl border border-slate-200 bg-white p-6 dark:border-slate-800 dark:bg-slate-900">
      ...
      <div className="grid gap-3 sm:grid-cols-3">
        {THEME_OPTIONS.map((opt) => (
          <ChoiceCard
            key={opt.value}
            label={opt.label}
            hint={opt.value === theme ? 'Đang dùng' : opt.value === 'dark' ? 'Mặc định' : undefined}
...
export default function Settings({ user, onUserUpdate, onNavigate, tab = 'account', theme, onThemeChange }) {
  return (
    <div className="mx-auto max-w-[720px]">
      {tab === 'account' && <AccountPanel user={user} onUserUpdate={onUserUpdate} />}
      {tab === 'contact' && <ContactPanel onNavigate={onNavigate} />}
      {tab === 'appearance' && <AppearancePanel theme={theme} onThemeChange={onThemeChange} />}
      {tab === 'language' && <LanguagePanel />}
    </div>
  )
}

// Thẻ lựa chọn (giao diện / ngôn ngữ). active = đang chọn; soon = chưa mở (khoá); onClick = chọn được.
function ChoiceCard({ label, hint, active, soon, onClick }) {
  ...
  return (
    <div
      ...
      className={
        ...
        (clickable ? ' cursor-pointer transition-colors hover:border-slate-300 dark:hover:border-slate-600' : '')
      }
    >
      <div>
        <div className="text-sm font-bold text-slate-900 dark:text-white">{label}</div>
        {hint && <div className="text-[12px] text-slate-500 dark:text-slate-400">{hint}</div>}
      </div>
      {active ? (
        <span className="flex h-5 w-5 flex-none items-center justify-center rounded-full bg-blue-600 text-white">
          <Check size={13} />
        </span>
      ) : soon ? (
        <span className="flex-none rounded-md bg-slate-200 px-2 py-0.5 text-[10.5px] font-bold uppercase tracking-wide text-slate-500 dark:bg-slate-700 dark:text-slate-400">
          Sắp có
        </span>
      ) : null}
    </div>
